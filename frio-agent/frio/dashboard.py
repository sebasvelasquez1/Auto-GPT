"""Frío dashboard — local, read-only, password-protected web view.

Shows the calculations in one place: product viability verdicts, the optimizer's
kill/scale proposals, the spend ledger, and the decisions audit log. READ-ONLY by
design — there are NO endpoints that spend money or launch/publish anything; those
stay behind their CLI gates + caps. Binds to 127.0.0.1 (local) by default.
"""
...
from .db.models import Competitor, Decision, MetricsDaily, Product, SpendLedger
...
def create_app(config: Config) -> FastAPI:
    ...
    def home(_=Depends(auth)) -> str:
        ...
        with Session() as s:
            ...
            proposals = propose_all(s, config, "creative")
            verdicts = [[{"kill": "🔪", "scale": "📈", "hold": "⏸️"}.get(p.action, "")
                         + " " + p.action, p.entity_id, p.reason] for p in proposals]
        ...
        body = (
            ...
            + "<p class=muted style='margin-top:18px'>El P&L por producto (analyzer) "
              "aparecerá aquí cuando conectemos costos reales + ventas de TikTok Shop.</p>"
        )
        return _page(body)
